...
import { analyticsApiPlugin } from "./vite-analytics-plugin.js";
...
export default defineConfig(({ mode }) => {
  // Vite only exposes VITE_-prefixed vars to client code by default; the analytics API
  // plugin runs as plain Node dev-server middleware and needs the *real* env vars
  // (DATABASE_URL, ANALYTICS_ADMIN_PASSWORD, ANALYTICS_SESSION_SECRET) from .env, so pull
  // them into process.env here — mirrors how those vars reach the Vercel functions in prod.
  Object.assign(process.env, loadEnv(mode, process.cwd(), ""));

  return {
    plugins: [react(), excelFolderPlugin(), analyticsApiPlugin()],
    base: "./",
