...
|--------------------------------------------------------------------------
|
| Exports are named "DD.MM.YYYY Dashboard.xls". We rank files by the date in
| the name — NOT the file's modified-time — because Windows copy/paste keeps
| the source's modified-time, so a freshly-pasted newer export can carry an
| older timestamp and get wrongly ignored. Returns a comparable YYYYMMDD
| number, or null when the name has no parseable DD.MM.YYYY date.
|--------------------------------------------------------------------------
*/
  ...
  return entries
    ...
    .map(e => {
      ...
      return {
        ...
        size: stat.size,
        dateKey: parseDateFromName(e.name)
      };
...
/*
 * Which of two files is "newer":
 *   - a dated filename always outranks an undated one;
 *   - two dated files compare by filename date, tie-broken by modified-time;
 *   - two undated files keep the original behavior (newest modified-time wins).
 */
function isNewerExcelFile(candidate, current) {

  const a = candidate.dateKey;
  const b = current.dateKey;
